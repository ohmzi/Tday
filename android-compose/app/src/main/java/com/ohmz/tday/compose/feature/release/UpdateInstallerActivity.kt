...
class UpdateInstallerActivity : ComponentActivity() {
    ...
    private fun handleStatusIntent(statusIntent: Intent) {
        when (statusIntent.getIntExtra(PackageInstaller.EXTRA_STATUS, Int.MIN_VALUE)) {
            ...
            PackageInstaller.STATUS_PENDING_USER_ACTION -> openInstallConfirmation(statusIntent)
    ...
    private fun openInstallConfirmation(statusIntent: Intent) {
        ...
        if (confirmationIntent == null) {
            Toast.makeText(this, getString(R.string.release_install_failed), Toast.LENGTH_LONG).show()
            finish()
            return
        }

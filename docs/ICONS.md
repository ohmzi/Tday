...
**Web** still picks list icons by hand and has no caller for the table. Adding one is a `buildWeb()` and a line in `ListIconTableExporter.outputs`, the day it grows one.

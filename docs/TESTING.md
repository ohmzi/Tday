...
| Today widget filtering, counts, setup/empty states, and refresh triggers | Unit | Medium |
...
Current XCTest coverage includes API model contracts, cache mapper date parsing, completed-sync merging, connectivity classification, realtime client behavior, server URL persistence, system credential login handling, and Today widget snapshot schema/status storage.
...
| Today widget snapshot schema, task cap, setup/empty states, and App Group key consistency | Unit | Medium |

...
- **Android**: `TodayTasksWidget` uses Glance with responsive compact, wide, and tall layouts. Small, medium, and large picker choices are separate AppWidget providers backed by shared Glance rendering and RemoteViews-compatible preview XML; Android 15+ generated previews are pushed from app startup. `TodayTasksWidgetModel` builds setup, empty, and tasks states from cached scheduled todos, exposes capped-row overflow, and `TodayTasksWidgetRefresher` lets cache writes refresh the widget after Local Mode and optimistic changes. Task layouts use a Glance `LazyColumn`, so rows scroll inside the widget whenever the visible viewport is too small; only model-cap overflow renders as a final `+N more` row. The widget add deep link opens a dedicated widget-create activity in the main app task so the Today create sheet appears without the main app Home/Today navigation stack; submitting hands task creation to a background submitter and returns to the launcher while the widget refreshes from cache.
- **iOS**: `TdayWidget` is a WidgetKit extension supporting `.systemSmall`, `.systemMedium`, and `.systemLarge`. The app writes schema-versioned snapshots to App Group defaults under `tday.widget.todayTasksSnapshot`; payloads include status, task count, generation time, and capped task rows. System-family WidgetKit widgets stay snapshot/glanceable because WidgetKit does not expose internal `ScrollView` widgets; T'Day fills the available family height with rows and shows compact overflow text based on total due-today count when more rows exist than the family can show or the stored cap includes.
- **Visual system fit**: each platform owns widget chrome, margins, background removal, tinting/accent rendering, and launch transitions. T'Day identity comes from the Today accent, rounded typography, priority dots, compact `N due` hierarchy, readable task rows, clearer add targets, and calm empty/setup states.

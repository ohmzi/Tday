...
- **Web asset/version cache (build-keyed invalidation)**: Each build stamps a unique `__BUILD_ID__` into the bundle and emits `/version.json`. The backend serves HTML and `version.json` as `no-store` and content-hashed `/assets/**` as `immutable`; the service worker serves navigations NetworkFirst; a client poller (`useVersionGate`) reloads into a new build (silently when idle, otherwise via a toast). This makes deploys self-healing and prevents stale-chunk crashes. Full design and developer guidance: [DEPLOYMENT.md → Web Cache Invalidation & Client Updates](DEPLOYMENT.md#web-cache-invalidation--client-updates).
...
- **Android**: `TodayTasksWidget` and `FloaterTasksWidget` use Glance with a shared internal visual layer for responsive compact, wide, medium, and tall layouts. Small, medium, and large picker choices are separate AppWidget providers backed by matching RemoteViews-compatible preview XML, but they are starting sizes only; each placed entry shares the same 2x2-to-4x4 resize range and adapts as it is stretched or compressed. Android 15+ generated previews are pushed from app startup. The widget models build setup, empty, and tasks states from cached scheduled todos or floaters, keep subtle empty-list-style watermarks behind every widget state, and refresh after Local Mode and optimistic cache changes. The Today watermark mirrors the app title icon by switching from sun to moon at night. Task states keep the header/add row fixed and use a scrollable Glance collection body with priority-colored bullets and capped rows. The widget add deep link opens a dedicated widget-create activity in the main app task so the matching create sheet appears without walking the app navigation stack. A third kind, `ListTasksWidget`, is per-instance configurable: its `android:configure` activity (`WidgetListConfigurationActivity`) lets each placed widget point at any single scheduled or floater list, rendering due-date-shaped or undated-shaped per the chosen list's type. Unlike Today/Floater's one snapshot file shared by every instance of a kind, its selection (`WidgetListSelectionStore`) and snapshot are keyed per `appWidgetId`, and `WidgetSnapshotWriter` rebuilds every configured instance on each cache write. See `android-compose/README.md`'s "List widget" section for the full design.

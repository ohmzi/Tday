...
- **Widgets**: Android Glance and iOS WidgetKit expose matching Today Tasks and Floater Tasks contracts. Today widgets show pending scheduled tasks due today and add through `tday://todos/create?target=today`; Floater widgets show active unscheduled floaters and add through `tday://todos/create?target=floater`.
...
- **Android**: `TodayTasksWidget` and `FloaterTasksWidget` use Glance with responsive compact, wide, and tall layouts. Small, medium, and large picker choices are separate AppWidget providers backed by shared rendering patterns and RemoteViews-compatible preview XML; Android 15+ generated previews are pushed from app startup. The widget models build setup, empty, and tasks states from cached scheduled todos or floaters, expose capped-row overflow, and refresh after Local Mode and optimistic cache changes. The widget add deep link opens a dedicated widget-create activity in the main app task so the matching create sheet appears without walking the app navigation stack.
- **iOS**: `TdayWidget` is a WidgetKit bundle supporting Today and Floater widget configurations for `.systemSmall`, `.systemMedium`, and `.systemLarge`. The app writes schema-versioned snapshots to App Group defaults under `tday.widget.todayTasksSnapshot` and `tday.widget.floaterTasksSnapshot`; payloads include status, task count, generation time, and capped task rows. System-family WidgetKit widgets stay snapshot/glanceable because WidgetKit does not expose internal `ScrollView` widgets; T'Day fills the available family height with rows and shows compact overflow text when more rows exist than the family can show or the stored cap includes.
- **Visual system fit**: each platform owns widget chrome, margins, background removal, tinting/accent rendering, and launch transitions. T'Day identity comes from mode accents, rounded typography, priority dots, compact count hierarchy, readable task rows, clearer add targets, and calm empty/setup states.

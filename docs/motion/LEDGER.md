# The motion programme ledger — 218 rows
...
```bash
grep -c '^- \[[ x]\] `' docs/motion/LEDGER.md   # 218 — must never shrink
grep -c '^- \[x\] `'    docs/motion/LEDGER.md   # rows closed so far
grep -c '^- ↳'          docs/motion/LEDGER.md   # 47 pointer lines into split rows
```
...
- [x] `root-feed-tab-switch-transition` — The root feed tab switch — the app's most-used interaction — is a hard cut on both native clients while the dock selector that triggered it springs across · and+ios · Impact O4 · S · Gate D+TF
  ...
  - **Both ticks stand; the length clause they assert does not, and PR 199 is where that is recorded.**
    `and-ios-root-feed-tab-swap-uncrossfaded`'s "One rung on both clients, and it is `Quick`" above, and
    `root-feed-tab-switch-transition`'s "the swap names `Quick` for itself — `RootFeedContent`'s
    `Crossfade` in `TdayApp.kt` and `AppRootView.swift:173`", were true when this block was written and
    are not now. Both clients run the swap on `Enter`, asymmetric, with the departing half raised, and
    there is no `Crossfade` left in the swap on either. Neither row is unticked: what both were filed
    against was the HARD CUT and the cut really did stop, so the tick is about the cut and still holds.
    What neither may be read as is a description of the current swap. See
    `and-ios-root-feed-tab-swap-not-web-route-fade`, which re-opens the specification and leaves both
    ticks in place.
...
### PR 32d — the Android tiles, the same transition built from nothing

- [x] `android-tile-zoom` — no tile on Android grows into its screen; every tap is the NavHost crossfade · android · Impact O4 · L · Gate D+G — iOS has had this since `PR 32` and the Android module had **zero** shared-element usage, so this is a build and not an extension: `SharedTransitionLayout` and `Modifier.sharedElement` were on the classpath (Compose 1.7.6) and unused. Ten surfaces: the six category tiles, the Today card, the scheduled list rows, the Anytime feed's Completed tile and its list rows.
  - **The key table is the iOS table, translated.** `AppRoute.tileTransitionKey(listId, highlighted)` is an exhaustive `when` with **no `else`** over all 21 routes, so a renamed or added route is a compile error rather than a transition that silently stops happening — the same property `AppRoute.zoomSourceID` gives iOS. `TileTransitionKeyTest` pins every key as a literal, pins that no two tiles share one, and pins that the two list routes need their id, so the strings are checked from the other side too. `AllTodos` answers null when a highlight resolved, exactly as iOS's does.
  - **The origin had to be invented, because Android's routes carry no argument for it.** iOS puts an optional `HomeTileOrigin` on four enum cases; Android's `AppRoute` is a plain object list, so the origin is a `savedStateHandle` flag that `NavController.navigateFromHomeTile()` sets on the entry it is leaving and the destination reads-and-removes — the same hand-off the All screen's highlight already uses. Polarity is **fail-closed: only a tile press sets the flag**, so a launcher shortcut, a notification tap, a widget row and the create flow's own push onto Today all arrive with no origin and claim no key. `TdayTileDestination(route, fromHomeTile, …)` gives `fromHomeTile` **no default**, so a destination that does not answer the question does not compile — the hazard this whole mechanism exists to prevent (a screen growing out of a tile the user never pressed) is a build failure rather than a quiet wrong animation.
  - **Both ends read the motion gate, which is the one thing a shared element can fail at alone.** `Modifier.tdayTileSharedElement` is a no-op when the key, the namespace or the scope is missing, or when `rememberTdayMotionEnabled()` is false — read at BOTH ends, so neither half can go inert while the other still animates. The namespace and the source scope travel in composition locals (`LocalTdaySharedTransitionScope`, `LocalTdayTileSourceScope`) rather than down a parameter chain, mirroring iOS's namespace-in-environment, because Compose 1.7 has no `LocalSharedTransitionScope` to read back from. Spec is `Springs.settle(visibilityThreshold = Rect.VisibilityThreshold)` — a named token, so no counter moved; the threshold is a unit rather than a motion value, and the clip is `OverlayClip(RoundedCornerShape(TdayDimens.RadiusCard))` so the growing rectangle carries the tile's own corners.
  - **Reduce Motion: a deliberate departure, recorded as one.** The three refused branches used to answer `EnterTransition.None` / `ExitTransition.None`, and now answer the same two curves on `Quick` — a refused hand-over still ARRIVES rather than cutting. `None` is not "no transition": it is the destination drawn finished on its first frame, which satisfies the fifth idiom rule, so this is **not** an application of that rule and `TdayApp.kt`'s KDoc says so outright, naming both things that encode the opposite (`route-handover.test.ts`'s three assertions and web's `animation: none` under `prefers-reduced-motion`) and pointing at the three branches as the whole of the change if the trade is not wanted. `route-handover.test.ts` moved in the same commit: two assertions became "the refused arm is on the shortest rung and does not travel" — the same still-frame property they always protected — and the leg-count one was **rescoped to the permitted arm**, because a third `tween<` leg on `Quick` is now deliberate rather than drift and counting across the whole declaration would have failed for the decision itself. Two mutations prove the re-aimed rules bite: moving a refused rung to `Emphasis` and putting a `scaleOut` back into the refused pop-exit each turn exactly one test red.
  - **The change introduced one hazard and it is fixed.** Zeroing a wait while the animation it covers still plays is the fifth idiom rule failing from the side nobody watches — and shortening the refused hand-over to 150 ms from nothing made that live at `ScheduledTaskHomeScreen`'s search-close hold, which was still scaling to 0 with the in-app switch. The wait now takes its **clock** from the system animator scale (the clock Compose actually runs the tween on) and its **rung** from the in-app switch: 260 ms over `Enter`, `Quick` over `Quick`, and 0 on a device that has removed animation — where the hand-over is 0 too, so the failure does not come back the other way.
  - **Where it differs from iOS, deliberately and on the record.** Both Completed tiles share one key, because Android's routes carry no origin argument to tell the two apart; the KDoc argues it — the two feeds coexist only for the 150 ms root-feed crossfade, during which no push can start — and iOS, which crossfades through a `ZStack`, needed two distinct ids for exactly that reason. That is a real difference between the platforms and not an oversight. Known limits left unproven: a source tile scrolled out of the `LazyColumn` before a pop means no zoom on that return, and the predictive-back seeked slot is the one interaction a device has to judge.

### PR 32e — the Android tile zoom, rebuilt so it reads like iOS

- [x] `android-tile-zoom-reads-like-ios` — the first cut of `PR 32d` shared the tile, so what grew into the screen was a copy of the tile's own icon and label · android · Impact O4 · M · Gate D — the report was exact: *"it looks like giant ui component coming out of the tiles"*. Compose draws the shared element's OWN CONTENT at both ends, so whatever node carries the key is what travels — naming the tile made the tile the payload.
  - **`ScaleToBounds`, argued from the library rather than from taste.** `Modifier.sharedElement` has no `resizeMode` at all in Compose 1.7.6; its reconciliation is `Constraints.fixed(animatedWidth, animatedHeight)` re-applied every frame, so each component keeps its absolute size inside a window that is still growing — a `titleLarge` toolbar and full-size rows crammed into a tile. That is the reported symptom, stated by the library. `sharedBounds(resizeMode = ScaleToBounds)` measures the content once against the lookahead (final) constraints and transforms only the window, so the layout is stable across frames and everything holds its relative geometry — and it is the only 1.7.6 API carrying both `resizeMode` and the `enter`/`exit` pair, so the mode and the fades come from one call.
  - **The travelling thing is a surface and nothing else.** The destination contributes a filled rectangle whose corners are clipped to a radius that starts at the tile's and ends square (`TdayTileCornerClip`), and THE SCREEN IS A SIBLING OF IT — `content()` laid out at its final size, fading in on the route's own enter. The screen is never inside the rectangle, which is the whole difference between this and a stretched screenshot of the screen, and it is what makes the arrival read as every component reaching its place at once.
  - **The source contributes nothing.** The ten tiles no longer carry the key on their `Card`: each wraps its card in a `Box` and publishes the rectangle from an empty `matchParentSize` sibling BEHIND it, so the icon, count, label and watermark are never inside a shared element, are never scaled, and leave on the route's own exit fade — the same `Enter` rung the screen takes to arrive, so the two blend rather than cut.
  - **The fixed corner clip is gone, and it was a bug.** The first cut clipped both ends to `RadiusCard`, which held a 26 dp radius over a full-bleed screen and popped square at the end of every open. With `ScaleToBounds` the content cannot overrun the animating rect, so the destination passes no clip and the source records nothing to clip.
  - **No new motion literal.** `Springs.settle` and `Rect.VisibilityThreshold` are unchanged; the two clocks are joined with `enter = fadeIn(Durations.Enter, Easings.Enter)` / `exit = fadeOut(Durations.Enter, Easings.Exit)` on both ends, so `verifyMotionTokens` stays a no-op and `TdayApp.kt` — and with it `route-handover.test.ts`'s assertions — is untouched. The origin gate is byte-identical and Reduce Motion is still the one `rememberTdayMotionEnabled()` read inside the shared bounds, so a refused motion installs neither end and the ordinary short fade plays.
  - **What a device has to settle, and the honest split.** The open is checkable and the failure modes are named. The CLOSE is not: the screen is now a plain child of the destination rather than something the shared element drew in place, so it takes the NavHost's pop exit — which has a quarter-width travel and a recede in it that the shared element used to suppress by covering it. Whether the screen slides sideways against the shrinking surface is the one thing this unit could not settle from source, and the fix would be a per-destination `popExitTransition` on the nine `composable(...)` blocks, which trades against the predictive-back argument `route-handover.test.ts` documents. That is a decision for eyes, not a machine, and it is on the device row.

### PR 32f — the Android tile zoom grows the screen itself, the way iOS does

- [x] `android-tile-zoom-grows-the-screen-itself` — the zoom still did not read like iOS: a translucent tile-coloured rectangle grew over a screen that was already fading in behind it at full size, and the close washed the whole screen in the tile's colour and slid it sideways · and · Impact O4 · M · Gate D+G — the report was *"Improve the animation of the android app when the list tile opens the screens. It should expand out of tiles into screen same way as iOS does."* It was recorded before anything changed — a Pixel 10 Pro XL emulator at 10x animator scale, `screenrecord` cut into frame sheets — and what `PR 32e`'s surface drew, in the tile colour `ed8cde96` later gave it, was the destination's content fading in across the whole viewport from the first frames with the rectangle growing over it; and on back, the screen washing to the tile colour before its content slid sideways under a shrinking rectangle. That second half is `PR 32e`'s device-row item (d), the one that row said it could not settle from source, observed failing.
  - **The screen is what travels now, which is what iOS's zoom is.** `.navigationTransition(.zoom)` lays the destination out at full size, draws it scaled into the pressed tile's frame and clipped to its corners, and springs the frame open. `PR 32e` kept the screen OUT of the shared bounds, as a sibling fading at final size, so no component would be drawn at a growing size — but `ScaleToBounds` guarantees that on its own: the content is measured once against the final constraints and only the window is transformed. So `TdayTileDestination` now composes `content()` INSIDE `sharedBounds(renderInOverlayDuringTransition = true)`, and what grows is a picture of the finished layout, never a layout at an intermediate size; the `PR 32e` report (*"giant ui component coming out of the tiles"*) has no path back in. `ed8cde96`'s message records the opposite belief — that clipping the screen to the animated rectangle "has no primitive in Compose 1.7.6". The primitive is the shared bounds itself: once the screen is inside it, `clipInOverlayDuringTransition` clips it, and `TdayTileCornerClip` (unchanged) supplies the radius. `ScaleToBounds(ContentScale.Crop, Alignment.TopCenter)`: `Crop` because a tile is far wider for its height than a phone and a fit would leave bands of the rectangle unpainted; `TopCenter` so what shows in a small rectangle is the toolbar and title, the part that answers to the label the user pressed.
  - **The surface is gone, and so is everything that carried its colour.** `TdayTileSurface`, `rememberHomeTileColor`, `TILE_TRANSITION_COLOR`, the colour parameter threaded through every tile click handler on both feeds (`CategoryCard`, the Today card, both feeds' list rows, the Anytime feed's Completed entry), the colour read in all nine destination blocks, and the one `TileTransitionKeyTest` case that pinned the colour hand-off. There is nothing left for a fill to be the colour OF: the tile's own `Card` stays painted in place, and the screen dissolves in over it inside the same rounded rectangle — a `fadeIn` over the first `Quick` of the spring while the rectangle is still near tile size, and on the way back a `fadeOut` delayed by `Quick`, so what shrinks is the screen until it lands and the dissolve happens on the tile rather than over home.
  - **Home holds still under the zoom, which no shared element can do by itself.** iOS opens the new screen OVER the old one. The NavHost crossfades every route, so home faded out while the tile grew and the zoom opened over an empty window. `exitTransition` and `popEnterTransition` now ask `isHomeTileArrival()` — of `targetState` on the push and of `initialState` on the pop, because the flag lives on the pushed entry — and answer `TdayTileZoomHold` on the push (opaque for `Scene`, then a `Quick` fade under a screen already covering it) and `EnterTransition.None` on the pop, so home is already drawn as the screen shrinks back onto it. The other two slots are untouched and only matter when there is no tile to match. `Scene` was measured, not assumed: a hold ending at `Emphasis` showed a strip of home fading along the top and bottom edges at 10x, where the rectangle's edges were still a few percent short of the screen's. `ExitTransition.KeepUntilTransitionsFinished` is the API that says this directly and it is internal in Compose 1.7.6, so the hold is spelled as the fade it ends in — and its KDoc says so.
  - **The close is a tween, because a back swipe seeks it rather than playing it.** Measured on this unit's first build by holding a back swipe at fixed points: the screen was half-way into the tile at 15% of the swipe and landed by 40%, and the rest of the drag showed nothing. `navigation-compose` drives the pop from a `SeekableTransitionState`, which maps the thumb's progress linearly onto the transition's total duration, and a spring's duration is estimated out to its visibility threshold — half a pixel here, about 630 ms for this one — so a time-linear seek spent the whole visible shrink in the first third. `TdayTileBoundsTransform` now picks by direction: the open, which nothing seeks, keeps `Springs.settle`; the close takes `tween(Durations.Emphasis, Easings.Enter)`, and so does the corner fraction, so the two still land together. Re-measured, the shrink moves through the whole swipe and lands at about three quarters of it; played by the back button it runs 66/91/100% at 100/200/300 ms against the spring's 51/89/99%, which the frame sheets cannot tell apart. Direction is the only thing the transform is handed that separates a seek from a press — `Transition.isSeeking` is set by the tooling's `seek` and never by `SeekableTransitionState`, and the NavHost's predictive-back state is private — and the KDoc says so.
  - **The origin flag moved onto the arrival, because the pop needs it too.** `PR 32d` wrote the flag on the entry being LEFT and had the destination read-and-remove it — right for a destination deciding once, useless to a pop that has to ask the same question again. `navigateFromHomeTile` now writes it on the entry it pushes, and both readers (`rememberHomeTileOrigin` at the destination, `isHomeTileArrival` in the NavHost) read it without consuming it, so push and pop get one answer for the life of the entry. The polarity is unchanged — only a tile press sets it — and the read is guarded on the entry's lifecycle, because the NavHost asks it of entries mid-transition and `savedStateHandle` throws on one not yet `CREATED` or already destroyed.
  - **Reduce Motion is the same gate, now on the NavHost's exception too.** `rememberTdaySharedScope` is still the one read both ends and the corner animation share, and both NavHost exception arms open `if (motionEnabled && …)`, so with motion refused nothing is installed, home is not held, and the ordinary short hand-over plays. Observed with the in-app switch on: both directions are the `Quick` crossfade in place, with no rectangle and no held screen.
  - **`route-handover.test.ts` moved, re-aimed rather than loosened.** Two assertions pinned `exitTransition = { navigationExitTransition(motionEnabled) }` and `popEnterTransition = { navigationEnterTransition(motionEnabled) }` as exact lines, which an exception arm breaks by construction. They now read each slot's lambda by balancing its braces: every slot still hands the gate to its ordinary transition, and every `if (` inside a slot must open `motionEnabled &&` — so the literal-`true` regression the test was written against is still caught, and so is an exception arm that skips the gate. The pop-enter slot still must not travel. A new test pins the two exception arms and the side of the transition each reads the flag from. Two mutations prove they bite: reading the flag off `initialState` in the exit turns the new test red, and dropping `motionEnabled &&` from the pop-enter arm turns it and the four-wirings test red, with `TdayApp.kt` restored byte-identical after each.
  - **Gates, run.** `cd android-compose && ./gradlew :app:assembleDebug :app:testDebugUnitTest` → the JUnit XML sums to 696 tests, 0 failures, 0 errors; `cd tday-web && npx vitest run tests/guardrails` → 41 files, 780 tests, all passing. No motion literal was written — every length, curve and spring is a token name — so `motion-parity.test.ts` did not move.
  - **What the emulator settled, and what it did not.** Recorded at 10x on this branch's debug build: the Scheduled grid tile, the Today card, a scheduled custom-list row, the Anytime feed's Completed entry and a floater list row all open as the screen growing out of the pressed rectangle over a home screen that does not move, and close as the screen shrinking back into it and dissolving as it lands — no colour wash, no sideways slide. A predictive-back swipe held at fixed points shows the screen part of the way back toward the tile in proportion to the thumb; released, it completes, and dragged back to the edge it grows back to full screen and stays. The pop exit's own slide and recede never reach the travelling screen, which is drawn in the shared overlay rather than inside the `AnimatedContent` child they transform. With the in-app Reduce Motion switch on, both directions are the short crossfade in place. Not settled: frame pacing on real hardware, which a swiftshader emulator cannot speak to; whether the close's tween and the open's spring read as one motion at full speed; and a source tile scrolled out of its `LazyColumn` before the pop — still the no-zoom return `PR 32d` named. All three are on the device row.
...
### PR 195 — the iOS docked title reserves for the title it actually has

- [x] `ios-docked-title-truncates-behind-the-mirrored-reserve` — the calendar's docked title ellipsised while the bar had 181pt of row going spare · ios · Sev 2 · M · Gate X+D — the report was the symptom and not the mechanism: *"when the title collapses and goes up in elastic transition, the title shrinks even though clearly theres space and goes like cale....."* Both halves of that are literal, and the arithmetic says so. This bar's docked title mirrored the wider control side on BOTH sides, so the two 56pt circles to its right — search and Today, both folded to circles by the time the title leaves zero opacity — spent 132pt twice out of the 357pt a 393pt phone leaves inside the bar's 18pt margins. "Calendar" is 137.5pt at the size it is drawn (32pt `Nunito-ExtraBold`, measured off `Resources/Fonts/Nunito.ttf`'s `hmtx` at wght 800), so it had 93pt; the `.minimumScaleFactor(0.72)` it leaned on still needs 99pt; and the bar's own controls occupy 176pt of the 357. Nothing was short of room except the reserve.
  - **The rule is ported, not invented, and the port is the whole change.** Android reached this first (`tdayBarTitleReserve`, `TdayHeroTitleHeader.kt:984`) and web followed (`nativePageBarTitleReserve`, `tday-web/src/components/app/nativePageBarTitleReserve.ts`) after the same bug was reported against its own calendar pill; iOS was the last client running the old gate and the only one of the three that never measured the title at all. `TdayBarTitleReserve.swift` is the third copy of that one rule — mirror the wider side while the measured title fits inside what that leaves, fall through to reserving each side for what actually sits there, and refuse to draw a title below `minWidth` — and `TdayBarTitleReserveTests.swift` is its twin of `TdayBarTitleReserveTest.kt` and `native-page-bar-title-reserve.test.ts`. The 12pt side gap stays iOS's own against the other two's 8: what is being ported is which SIDES to reserve on, and moving the gap would move the title on every bar that already fits, which is the one thing this rule must not do.
  - **It cannot cost any bar width, and that is the property that makes it safe to land without a device.** Per-side leaves `W − L − T − 2g` and mirrored leaves `W − 2·max(L,T) − 2g`, so per-side beats mirrored by exactly `|T − L|` — never negative. The rule only ever moves a bar from mirrored to per-side, never the other way, so every bar is handed back at least the room the old gate handed it. `TdayBarTitleReserveTests` pins that as a property over 9 bar widths × 9 cluster widths × 7 title widths rather than as examples, and pins with it the three claims that make the change reversible in review: a title that was drawn before is still drawn, a reserve never reaches under a control it is meant to clear, and an unmeasured title takes the mirrored branch exactly as the bar always did — so the first frame is the old behaviour and the second is the fixed one.
  - **The cost is the crossfade's centre line, and it is paid only here.** The mirrored branch is first and stays first because it keeps the docked copy centred on the bar, which matches the expanded copy centred on the screen; the per-side branch centres it on the leftovers, so it sits `(trailing − leading) / 2` — 32pt on this phone — to the left of it over progress 0.72–0.86, where both copies are on screen. That is real and it is the thing the old comment existed to prevent. It is still the right trade and the reason is measurable: the mirrored reserve is what put the two copies at DIFFERENT SIZES over that same window, the expanded one at 32pt and the docked one at the 0.72 floor, which is a worse disagreement than a lateral one; past about a 437pt device the mirrored reserve fits and the branch is never taken at all; and it is what Android and web both chose in as many words. The comment at the call site no longer claims the scale factor stood in for the arithmetic, because it did not.
  - **`hasRoom` arrives with the rule, and it is the one parity piece iOS was missing outright.** Android hides the docked title below `DockedTitleMinWidth` (`TdayHeroTitleHeader.kt:657`) and web does the same at `nativePageBarTitleReserve.ts:145`; iOS painted a stump. It cannot engage on this bar — "Calendar" against 357 − 200 = 157pt — but it is the gate that makes the fall-through safe on a bar where the controls really have eaten the row, and without it the rule would be a different rule from the other two clients'.
  - **The measurement is iOS's own precedent and not a new mechanism.** A hidden `Text` copy at `fixedSize()` — so it answers with the title's natural width rather than the width the bar leaves it — reporting through a `PreferenceKey` into `@State`, hung as a `background` so it can move nothing. `RootFeedHeroHeader.swift:206-212`/`:282-284`/`:384-390` is the same three pieces already in this tree on the same problem, and `TdayFont.uiFont` already resolves the family for anyone measuring off the render path, which is what makes the reserve testable as arithmetic. `ViewThatFits` was the alternative and was not taken: it would carry no number, and a rule three clients share is one that has to be readable against itself.
  - **What a device has to settle.** The reserve is arithmetic and `TdayBarTitleReserveTests` is the proof; the crossfade's 32pt lateral shift over 0.72–0.86 is not arithmetic and is on the device row. So is the footer of the report that started this — the same bar's expanded and docked copies agreeing about size while both are on screen.

### PR 196 — the fade band stops painting over the bar it belongs to

- [x] `ios-fade-band-cut-the-back-buttons-shadow` — the bar's bottom fade band was an overlay, so it was drawn after the back button and guillotined the button's shadow at the bar's own edge · ios · Sev 1 · S · Gate D — *"the shadow behind the back buttton in ios is cut off under when the toolbar shrinks and underneath screen comes up, blend the shadow nicely so it doesnt look like sudden cut off"*. The band is the bar's own dissolve: a 30pt `LinearGradient` from `colors.background` to clear, offset a whole `contentFadeHeight` below the bar so rows passing under its edge dissolve instead of being guillotined there. It was attached with `.overlay(alignment: .bottom)`, which paints it on top of the bar's OWN content — including the 56pt back chevron, whose shared press shadow is black at 0.15 on a 16pt radius with a 10pt drop, and therefore reaches about 26pt past a frame the bar only pads 2pt below. So the band's fully opaque top edge, which sits exactly on the bar's bottom edge and is at full strength from collapse progress 0.125, landed across the shadow while the button was still drawing it.
  - **A late-collapse defect, which is why the report says "when the toolbar shrinks".** The bar holds ~200pt of mark block, spacer and expanded title under the button row until the collapse is nearly done, so the shadow is comfortably inside the bar for most of the scroll and the two only meet once the bar is down to its docked height — 60pt, the button row plus 4pt of padding. The band is already fully opaque by then.
  - **Paint order was the fix, not geometry, and that is Android's own finding one client over.** Android's band was a Column sibling drawn after its bar and did exactly this; commit `054d8184b` moved it to be drawn first and offset below, and its comment says the same thing this one now does — *"nothing moves, so the band still starts at the bar's edge ... but the shadow now falls on top of the band and fades out with it."* So the one-token change here is `.overlay(alignment: .bottom)` → `.background(alignment: .bottom)`: a background paints behind the modified view's own rendering while still rendering outside its bounds, so the band keeps its rectangle, its offset and its opacity ramp — it only stops covering the bar. Applied to both iOS bars that share the idiom — `CalendarElasticTopBar` in `CalendarScreen.swift` and `TimelineTopBar` in `TodoListScreen.swift`, each at its own `.background(alignment: .bottom)` — because they share the defect and the argument.
  - **The third client is not the same shape and is left alone.** Web's `NativePageHeader` has always painted its band below the bar's box, but its DOM order puts the back button and the actions BEFORE the band, so its own bar is exposed the same way (`rootFeedHeaderButtonClass`'s `shadow-[0_14px_30px_-16px]` against `pb-1.5`); the calendar page escapes it because that page renders `MobileSearchHeader`, whose band precedes its leading block. That is a web change with its own blast radius and it is not this row.
  - **What a device has to settle.** Nothing here is pinnable by a machine: a paint order is settled by looking. The band must still dissolve the rows passing under the bar's edge — it should, since the whole bar subtree including its backgrounds composites above the list — and the shadow must now read as fading out under the bar rather than stopping at it. Both are on the device row.
  - **Also owed by this row and not provable here:** the iOS 18 clause in the report. `CalendarElasticTopBar` is inside a matched-transition destination (`ZoomNavigation.swift`, `navigationTransition(.zoom …)`), so a cut seen during or just after a zoom push would be hosted by the transition rather than by the band, and the band's opacity window coincides with it. The discriminating check is on the device row.

### PR 197 — the iOS tiles' square edge, traced to the zoom and not to the tile

- [x] `ios-tile-square-edge-is-the-zoom-source` — the tiles' rounded fill left the screen with a square corner, reported against "ios 18" · ios · Sev 2 · S · Gate X+D — the report was *"these tiles are round in the corner but they have a pointy square shadow or outline so it doesnt look really round, the colour is round but then theres square edge"*, and the audit's answer is that the square was never the tile. Nothing on any of the ten surfaces can draw one at rest: the base of every one is the same `RoundedRectangle` the clip uses, `clipShape` precedes the shadow in all ten chains, and the zoom source was the only iOS-18-gated modifier on exactly these five `.tdayZoomSource` call sites — `matchedTransitionSource(id:in:)` called with NO configuration, which the engine reconciles by clipping the travelling source to a plain rectangle.
  - **The FAB is the discriminator, and it is why the first fix was wrong.** `.compositingGroup()` went on between the clip and the shadow at all ten, on the argument that a multi-layer label's shadow is taken from the frame those layers sit in. It is not: the FAB is `.background(fillColor)` — an UNSHAPED rectangle — then `.clipShape(Circle())` then this same `TdayPressEffect`, at 0.24 black on a 16pt radius. If a shadow were sampled from anything but the clipped alpha, that 56pt circle would cast a square on every screen in the app and would have been the first thing anyone reported. So the modifier was inert where it was not a cost, and it was removed rather than landed. `ScheduledTaskHomeScreen.swift` keeps a short note where the long one was, because the tile is where the next reader would be tempted.
  - **The cost was not zero, which is what made it worth taking back out rather than leaving in.** The Today card and the six category tiles run `.contentTransition(.numericText(value:))`, so a compositing group re-rasterises their fills, gradients, watermark and label on every frame of the count roll. `.compositingGroup()` appears twice in the whole tree and both are drag previews whose base layer is an unshaped `.background(colors.surface)` AND which carry `.opacity(0.96)` — the ancestor compositing effect the modifier is documented for. It is the substitute for a shaped background, not a shadow remedy.
  - **What DID need a single answer was the rung, and that is the half that stayed.** `TdayRadius.card` (26) already existed from the zoom half; six sites across four files still spelled the number themselves — the board's tiles and Today card and list row, the Anytime feed's two cards, the wizard's two tiles and the reset flow's hero tile — and they name it now, which is what Android already does (`TdayDimens.RadiusCard`). Web writes `rounded-[26px]` at each of its tile sites rather than naming it, so the comment on the token says the three clients draw one value and only two of them name it.
  - **`.listRowBackground(colors.background)` is settled and was NOT changed.** It is the one literally rectangular surface drawn under any of the ten, and it cannot be seen: `colors.background` is opaque with no alpha, and the List it belongs to hides the system background and paints that same expression itself, so the rectangle is drawn on top of itself. It is also the file's uniform idiom across thirteen sites; changing two of them would make these the only rows arguing about their own background. What it does mean is that the two expressions have to move together — if the List's background ever becomes a gradient, those two rows become a visible grey rectangle behind the cards.
  - **The one rectangular boundary left standing on the ten, and it is not the shadow's.** The two Anytime cards are the only two of the ten drawn as `List` rows, and their row insets are `top: 0, bottom: 10` against a shadow with 10pt of radius at a 7pt drop. SwiftUI clips row content to the row, so the top inset leaves the card's own shadow no room above it and the bottom one truncates a reach that is longer than 10. A round silhouette can still be cut in straight lines there, and no shadow path can escape a row clip — so if the Anytime half of the report survives, that boundary is what is left and the fix is vertical room on those two rows or lifting them out of the `List` the way the scheduled board draws its tiles. It is on the device row, not in this change.
  - **Android's half of the citation was wrong and is corrected rather than repeated.** `TdayTileCornerClip` does not "hold the tile's radius at the tile and square at the screen" as a fixed pair: it re-derives the radius on every draw, 26dp at the tile and 0 at the screen (`TdayTileTransition.kt`), which is why Android's arrival squares off cleanly and Android's own row calls a radius held over a full screen and then popped square the bug its rebuild removed. The iOS configuration closure has nowhere to put a moving one — it configures a static source clip — so the destination's behaviour is on the device row instead of asserted in the comment. `IOS 17` is unaffected by all of this: the whole modifier is inert below 18 by construction.
  - **What a device has to settle, and one of them is a three-second check.** Whether the FAB's shadow is round — if it is square the mechanism is real and app-wide, and the fix belongs in `TdayPressEffectModifier` rather than on ten surfaces. Whether the Anytime cards still show a straight edge from the row clip above them. Whether the corner snaps square as a zoom lands. And the `matchedTransitionSource` configuration closure is the one construct in this diff that no local gate can typecheck — `launch-handover.test.ts` asserts only that the string sits inside an availability block, never the call's arity or closure shape — so it wants a build against an iOS 18 SDK.

### PR 198 — one glyph for the Scheduled feed and the history page it opens

- [ ] `scheduled-dock-and-history-mark-share-one-glyph` — the Scheduled tab was drawn as a house and the Completion-history page as a circle-check, two drawings for one destination · all · Sev 1 · S · Gate D — the report was *"the home icon for scheduled task should be updated to something new something more along with line of calendar, or checklist or the app icon to represent schedulded instead of home. and use the same icon in the completion history icon in the page."* The direction chosen was a calendar with a tick in it — Lucide's `calendar-check`.
  - **The glyph is vendored, not drawn, and that is the one thing that could have gone wrong quietly.** The path data is taken from `tday-web/node_modules/lucide-react/dist/esm/icons/calendar-check.js` — `M8 2v4`, `M16 2v4`, an 18×18 `rect` at `(3,4)` with `rx:2`, `M3 10h18`, and the tick `m9 16 2 2 4-4`. Android is the only client that needs the conversion work, because a vector drawable takes paths and no `rect`: the body is written with the same path data `ic_lucide_calendar` already uses, and the tick is absolutised as `M9 16 11 18 15 14` — the same three points the relative command walks, which is how `ic_lucide_circle_check_big` writes its own. Web needs no asset at all; `CalendarCheck` is already in `lucide-react` and the change there is an import.
  - **Both Android call sites, because there are two and only one of them is reachable.** `RootFeedDock.kt` names the dock glyph twice — `:175` for the expanded control and `:531` inside the item, behind an `if (tab == SCHEDULED_TASK_HOME)` guard. Changing only `:175` changes nothing at all; changing only `:531` leaves the expanded control on the house until someone removes that guard, at which point the old glyph silently returns. They are one glyph and now name one drawable.
  - **iOS had a second, latent copy of the same problem.** `RootFeedTab.systemImage` returned `"house.fill"` and is drawn only because the collapsed pill's `else` branch reaches for it; the pill actually draws the bundled `NavHouse`. That symbol is gone — `systemImage` is now optional and returns `nil` for Scheduled, so a caller that ignores the optional draws NOTHING rather than the old house. Nil rather than a name is the point.
  - **What deliberately did NOT change, and each is a decision rather than an omission.** The Completed dashboard tile keeps `circle-check-big` — `docs/ICONS.md`'s tile row is about the *tile*, and the ask was about the page's own mark, so the tile and the page are two glyphs on purpose; the new paragraph in `docs/ICONS.md` rules the dock-tab-to-page pair together and does not extend it. `ic_lucide_house` / `NavHouse` / `LucideHouse` all stay, because they are list icons: the `home` key is in the shared registry and the keyword "apartment" infers it, and `ListIconInferenceKeyParityTest` is the three-way gate that would fail if they went. And the car's Today button still wears an SF Symbol on iOS — that whole bar row (`house.fill`, `leaf`, `plus`) is symbols, so converting one of the three would be a half-measure; it is recorded here rather than fixed. Android's car surface IS changed, because there both glyphs are drawables and it is the dock's own pair.
  - **No guardrail covers this, in either direction.** `guide-icons.test.ts` and `settings-icons.test.ts` assert existence only for their own 35- and 18-glyph fixtures, and the two list-icon tests lock the list registries — so nothing would have gone red if the new asset were misspelled or absent, and nothing goes red now that it is right. The one real tripwire is about REMOVAL, not addition. That is why the device row exists and why the glyph's provenance is written down here.
  - **A rendering note the reviewers raised and the device row is for.** `calendar-check` is denser than the circle-check it replaces: its only fine detail is a 6×4-unit tick inside a 24-unit box, and at the dock's 22pt that aperture is small. It was computed to survive, and the history hero's 108pt echo at 0.17 alpha now reads differently — a rectilinear calendar under a circular clip is cut into bars rather than the concentric arcs the circle-check produced. That is a look, not a defect, and it is what the row asks to be judged.

### PR 199 — the root feed tab swap takes web's route fade, on both natives

- [x] `and-ios-root-feed-tab-swap-not-web-route-fade` — the root feed tab swap is faded, but not web's fade: an in-place SYMMETRIC crossfade of the feed body alone on `Quick`/`Standard`, where web runs the arriving screen in on `Enter` and the departing one out on `Exit`, both over `Enter`, with the departing half painted above · and+ios · Impact O3 · S · Gate D+TF
  - **It cross-references two ticked rows and takes neither of them back.** `and-ios-root-feed-tab-swap-uncrossfaded` and its twin `root-feed-tab-switch-transition` were filed against a HARD CUT and closed when the cut stopped being a cut: the swap really did change in one frame, and it really is faded now. Both rows stay ticked. What this row re-opens is the SPECIFICATION their sub-bullets went on to assert — `:566`'s "**One rung on both clients, and it is `Quick`**" and `:573`'s "the swap names `Quick` for itself — `RootFeedContent`'s `Crossfade` in `TdayApp.kt` and `AppRootView.swift:173`" — because that length was already contradicted by the normative table those rows belong to: `docs/motion.md`'s `Scene` bullet says "Nor is this rung for route or tab handovers: those are `Enter` on both clients that have them", and `TdayMotionTokens.kt`'s doc string repeats it. What the ticked pair delivered was a fade. What it did not deliver is web's fade.
  - **The spec, quoted from the source, is two curves with the departing half on top.** `tday-web/src/globals.css:1468` — `.tday-route-fade { animation: var(--tday-duration-enter) var(--tday-ease-enter) both tday-route-fade-in; }` — brings the arriving screen in; `:1501` — `::view-transition-old(root) { z-index: 1; animation: var(--tday-duration-enter) var(--tday-ease-exit) both tday-route-fade-out; }` — takes the departing one out over the same Enter length, on the Exit curve, and raises it. `:1488` states the read: "a fade THROUGH the background rather than a straight crossfade — the new content reaches half opacity a beat after the old has left half of its own." A `Crossfade` cannot express it at all — one `animationSpec` goes to both children, so its halves share a clock and a curve by construction, which is what `TdayApp.kt` said in as many words before this change.
  - **Android is two `AnimatedVisibility`s in the same `Box`, and the ordering is `Modifier.zIndex`.** `RootFeedContent` no longer holds a crossfade: each feed carries its own visibility keyed on `rootFeedTab ==` its tab, a shared `fadeIn(tween(Durations.Enter, Easings.Enter))` for the half that arrives and `fadeOut(tween(Durations.Enter, Easings.Exit))` for the half that leaves. Both specs are token names; no literal was written. `AnimatedVisibility` keeps its content composed for the whole exit, so the two-live-layers property the old comment protected is intact, and the create-task sentinel still means what it meant — `if (scheduledFeedSelected) rootCreateTaskRequestKey else 0` — only restated per feed. The departing feed asks for `Modifier.zIndex(1f)` off `rootFeedTab !=` its own tab, which is the only question a feed's own body can answer about whether it is on its way out, and a `Box` orders its children by zIndex: web's `z-index: 1` is reproduced directly.
  - **iOS is `.transition(.asymmetric(insertion:removal:))` per arm, and the ordering had to come from the transition.** Each feed carries `insertion: .opacity.animation(TdayMotion.enter(duration: Durations.enter))` and `removal: .opacity.animation(TdayMotion.exit(duration: Durations.enter))` — the same one-length/two-curve pairing the launch splash already uses, which is where this file had already recorded that "SwiftUI has no way to carry two curves through one `.animation(_:value:)`, so the curves live per-arm". The difference is that a splash arm only ever moves one way and a feed arm moves both, which is why this is `.asymmetric` and the splash is not. The raise could not be a `.zIndex` written inside an arm: a feed is only ever BUILT while it is the selected tab, so the departing view is the render it had before the state changed and anything written in an arm reads the same for both halves. It comes from `AnyTransition.modifier(active:identity:)` on the removal side (`TdayFeedDeparture`), which is the one API that is told which half it is on. **The raise orders the two FEEDS against each other and against nothing else, and the chrome is now explicit about that rather than relying on declaration order.** `rootFloatingControls` carries `.zIndex(8)` — the twin of Android's `Modifier.zIndex(8f)` on the dock and the create button, against the feeds' `0f`/`1f` — so the chrome is above both feeds in every state, the one the raise is live in included. Without it a sibling at the `ZStack`'s implicit 0 would be painted under an opaque full-screen departing feed for the whole 200 ms, and the dock and the create button would go dark at the first frame of every swap. That is the one property of Android's shape that did not come across with the ordering itself.
  - **Reduce Motion is a clean cut on both clients, and on Android it is a clean cut by construction.** Motion off passes `EnterTransition.None` / `ExitTransition.None` on Android and `tdayAnimation` returns nil on iOS, so the arriving feed is drawn finished in the frame the tab changes — the fifth idiom rule, and what both clients already did. The half web documents and removes outright is the departing layer left opaque ON TOP: `globals.css:1509`, "an un-animated `old` is opaque and on top, and would hold the screen the user just left for the frame it takes the transition to end." Android cannot fall into it, because with no exit spec `AnimatedVisibility` disposes that content in the same frame; iOS's raise only ever applies inside a removal transition, so with no animation there is no window for it to apply in. Both are on the device rows as the failure to look for.
  - **The rung moved to `Enter`, and the reversal is stated rather than slipped in.** Both clients now time the body on `Durations.Enter` with `Easings.Enter` / `Easings.Exit`. The `Quick` plus `Standard` pair both comments used to argue for — on the grounds that the body follows a control rather than being one — is gone, and both comments now say what replaced it and that `docs/motion.md` had already answered the argument in prose. Android's create button accent moved with the body to `Durations.Enter` and keeps `Easings.Standard`: it is one property crossing one colour, with no arriving or departing half for a directional curve to describe.
  - **The iOS create-task hand-off stopped leaning on a sleep, but it is NOT the fix Android got, and the first draft of this bullet said it was.** `AppRootView.swift` handed BOTH feeds `createTaskRequestID: rootCreateTaskRequestID` and relied on the fade finishing inside `presentPendingRootCreateTaskIfReady`'s 180 ms sleep — which `Quick`'s 150 ms did and `Enter`'s 200 does not, so the sleep had to stop being the guard. What replaced it is the `0` sentinel each arm passes on the tab it is not (`(rootFeedTab == … ? rootCreateTaskRequestID : 0)`), and that ternary is **unreachable where it is written**: it sits inside a `switch rootFeedTab` case, so the arm is only ever built while `rootFeedTab` already equals the tab it tests and the condition is a tautology at the only moment it is evaluated. **The load-bearing half on iOS is the frozen render.** The departing copy is the body it had before the tab changed — the same premise `TdayFeedDeparture` rests its whole z-order argument on, and the same one `AppRootView.swift`'s own doc states — so its `createTaskRequestID` never changes and its `.onChange` cannot fire. The sentinel is kept as defence in depth and costs nothing: it is the correct value under the other reading of that premise, and both screens' `guard requestID > 0` refuses `0` whichever reading holds. Android's shape really is structural, because `AnimatedVisibility` recomposes its departing content with the new argument (`TdayApp.kt:1383-1426`) — the two clients reach the same guarantee by different routes, and only one of them reaches it through the sentinel. The sleep stays as a settle before the sheet appears, and its comment now says it is timing rather than the protection. **A device is what settles the premise**: the iOS row asks for the deep link twice and says plainly that two sheets is a finding about the frozen render rather than a test flake.
  - **No guardrail covered the swap, before or after.** `route-handover.test.ts`'s Android half reads `TdayApp.kt` as text but asserts only about the NavHost (`navigationEnterTransition` / `navigationExitTransition` and their four wirings), and nothing in `motion-reachability-android.test.ts` or `motion-reachability-ios.test.ts` keys on the swap. Both web gates were run and are unchanged, as they must be: `cd tday-web && npx vitest run tests/guardrails/` → 36 files, 719 tests, all passing; `cd tday-web && npm run test` → 151 files, 1515 tests, all passing. Android: `cd android-compose && ./gradlew :app:compileDebugKotlin :app:testDebugUnitTest` → `BUILD SUCCESSFUL in 12s`, and the JUnit XML under `app/build/test-results/testDebugUnitTest/` sums to 683 tests, 0 failures, 0 errors. iOS is textually complete and delimiter-balanced against HEAD; there is no local iOS compile or xctest, so `X` is owed and this row leans on `D` for its look.
  - **What a device has to settle, one row per client**, appended to `docs/verification/phase-9-device-pass.md` unticked — the asymmetry is a look, the z-order is a look, and the Reduce Motion half is a look whose failure mode is a HELD SCREEN rather than an error. Two more are on the rows and both exist because of a reviewer finding rather than because the code looked wrong: whether the dock and the create button stay fully drawn through the hand-over (the chrome's own z-order, which is the one part of Android's shape that had to be re-stated on iOS rather than inherited), and whether the create deep link opens exactly ONE sheet (which is the only observable that can tell the frozen-render premise this file reasons from apart from the other reading of it).

### PR 200 — one mark for the Completion-history page

- [ ] `history-mark-is-one-green-check-over-leaf-and-calendar` — the page's mark was a single glyph on a disc, and the green the user arrives through was not the green the page showed · all · Sev 1 · M · Gate D — the report was *"the greencheck icon should be same colour not different, but it should have leaf and scheduled icon in the background along with greencheckmark. produce something nice."* The arrangement chosen was stacked-behind-faint.
  - **The colour is the tile's, and that is the point of the row.** Four different values were drawing "the Completed green check" across the three clients — `#5E6878` for both natives' page mark, `#719F84` for web's page mark and every client's tile, `#6FBF86` for both natives' row tick and `#46B963` for web's. The mark now takes `#719F84`, because that is the colour of the tile the user tapped to get here and it is already named on all three (Android `TdayCompletedTileAccent`, web `nativeScreenAccentColors.completed`, iOS a call site). iOS had no shared token for it — one file-private constant and one call site, and `Color(hex:)` is `private` per file — so `Colors.swift` gained `tdayCompletedGreen` rather than a third hex.
  - **Three glyphs at one size fuse, and that is measurable rather than a matter of taste.** Drawn concentric at full size the leaf's contour sits one stroke-width inside the calendar's frame and the two outlines read as one fringe at 44pt and at 300px alike. The back plate is now graduated — check 1.00, `calendar-check` 0.88, leaf 0.62 — which puts `9 × (0.88 − 0.62) = 2.34` units between the two right-hand extremes against the `0.62 + 0.88 = 1.50` units of half-width the strokes carry: **0.84 units of clearance, 1.54pt at the hero's 44pt**, holding at every size. Scaling thins the strokes as it goes, so depth reinforces.
  - **What the mark loses, disclosed rather than hidden.** The `calendar-check`'s inner tick cannot be read behind the front check: the check's arms sit on `x+y = 26` / `−x+y = 8` and the tick's on `x+y = 29` / `−x+y = 7`, leaving separations of 2.12 and 0.71 units against the 2.0 the two strokes carry, so the tick is subsumed. It was attacked twice — the reviewer's arithmetic independently re-derived and confirmed — and both escape routes were rendered and rejected: shrinking the check below `s = 0.52` makes the tick legible but the mark smaller than its own detail, and offsetting the calendar satisfies both arms but leaves the disc visibly lopsided, or 0.44pt of clearance at the minimum-norm offset, which buys the inequality and not the legibility. What survives behind the check is the calendar's frame, its header rule and both binding ticks, and all three code comments say so.
  - **A blocker the reviewers caught, and it was a rendering bug rather than a style note.** Android's composite passed `tint = Color.Unspecified` to its three `Icon`s on the assumption that it means "inherit". It does not — Material3 compiles it to a null `ColorFilter`, so the vendored white-stroked drawables painted white at the host's alpha: the page watermark was invisible on the light theme and the badge was white by accident. Verified against the artifact rather than the documentation (`javap` on material3-android 1.3.1: `equals-impl0(tint, getUnspecified()) → aconst_null`), and fixed at the one place it can be, by resolving `LocalContentColor` inside the composite so the parameter means what the components around it promise.
  - **The shared components gained defaulted slots, so nothing else moved.** The mark-bearing components on each native client take a glyph rather than a view, and they are shared far past this page — six iOS callers of `TimelineExpandedTitleRow`, twelve web callers of `NativePageHeader`, and `TdayEmptyState`/`EmptyTaskWatermark` on Calendar, the board and the feeds. Each got a slot defaulted to its existing behaviour, so a baked-in composite could not have put the history mark on the Today feed. The hero disc's echo gained a colour parameter too: on both natives it was repeating the mark's own check in the screen's slate chrome, which made the mark hold two colours where web held one.
  - **Deliberately not decided here.** The row's restore toggle stays on its own green (`#6FBF86` natives, `#46B963` web) rather than the mark's `#719F84` — that is the tile-versus-screen accent question, which PR 201 owns, and this row hands it the numbers rather than settling it. Web's **Floater** history tab keeps the Floater teal: that tab's page accent is teal on all three clients and native has no Floater history screen to be parity with, so forcing green there would have decided an accent question silently.
  - **What a device has to settle.** Whether the leaf reads as a leaf behind the calendar at the smallest size the mark is drawn, and whether the graduated back plate reads as depth rather than as a smudge. The arithmetic says the outlines clear; only an eye can say whether they are recognisable.

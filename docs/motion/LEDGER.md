...
- [ ] `and-hero-circle-buttons-press-scale-unanimated` — two hero buttons compute a 0.93 scale from a bare `if (pressed)` inside `graphicsLayer`, and the `PressableIconButton` written for exactly this has zero call sites · and · Sev 1 · S · Gate G+D — **half landed**: both buttons are on `Modifier.tdayPressable` at `PressScales.Bar`, so the animation exists; the row still waits on `PressableIconButton`, which is the other half of the defect and is still sitting in `ScheduledTaskHomeScreen.kt` with no callers

...
### PR 40b — the Android skeleton

- ↳ part 2 of 3 of `skeleton-loading-vocabulary` — `TdayTaskRowSkeleton` + `TdayTaskRowMetrics` at `TodayTodoRow`'s own geometry; the timeline's `surfaceVariant` card and Completed's `displaySmall` word both retire; JVM test. Box lives under **PR 40a/b/c**.
...
- [ ] `skeleton-loading-vocabulary` — one skeleton per client at real row geometry, handed over on the `Enter` rung · all · Impact O4 · M ea · Gate V/J/X

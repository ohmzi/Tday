...
- ↳ part 5 of 6 of `reduced-motion-coverage` — root `@Environment` read + custom `EnvironmentKey` + `tdayAnimation(_:)`. Box lives under **PR 35b**.
  ...
  - **No new motion literals.** Nothing here declares a duration, a curve or a spring, so the three budgets are untouched. The two `easeInOut` literals in `TdayEmptyState` lost their `guard` and gained a `tdayAnimation(…)` wrapper; both numbers are where Phase 6 left them.
